...
function buildNextCart(cartItems, flavor, nicotineType, strength, quantity) {
  ...
  const existingItem = nextCart.find((item) => item.flavorId === flavor.id && item.strength === strength && item.nicotineType === nicotineType);
...
function getRemainingResources(cartItems, data) {
  ...
  data.flavors.forEach((flavor) => {
    ...
    remainingFlavorsById[flavor.id] = Math.max(0, Number(flavor.quantity) - used);
  });

  return {
    flavorsById: remainingFlavorsById,
    inventory: {
      salt: Math.max(0, Number(salt?.quantity || 0) - result.totals.totalNicotineUsageByType.salt),
      nicotine: Math.max(0, Number(nicotine?.quantity || 0) - result.totals.totalNicotineUsageByType.nicotine),
      base: Math.max(0, Number(base?.quantity || 0) - result.totals.totalBaseUsage),
      bottles: Math.max(0, Number(bottles?.quantity || 0) - result.totals.totalBottles),
    },
  };
}

function getFlavorRecipeCapacity(flavorId, nicotineType, strength, remainingResources) {
  ...
  const availableFlavor = Number(remainingResources.flavorsById[flavorId] || 0);
  ...
  const availableNicotine = Number(remainingResources.inventory[nicotineType] || 0);
  ...
  ['salt', 'nicotine'].forEach((nicotineType) => {
    [6, 12, 18].forEach((strength) => {
      maxPossible = Math.max(
        maxPossible,
        getFlavorRecipeCapacity(flavorId, nicotineType, strength, remainingResources),
      );
    });
  });
...
function updateClientOrderAvailability() {
  ...
  const data = getData();
  const submitButton = document.querySelector('#clientOrderForm button[type="submit"]');
  const nicotineType = document.getElementById('clientOrderType')?.value;
  const strength = Number(document.getElementById('clientOrderStrength')?.value);
  const quantity = Number(document.getElementById('clientOrderQuantity')?.value || '1');
  ...
  const maxPossible = getFlavorRecipeCapacity(activeOrderFlavorId, nicotineType, strength, remainingResources);
  ...
  if (maxPossible <= 0) {
    ...
    setClientOrderMessage(`Nie można dodać: ${result.shortages[0] || 'brakuje materiału.'}`, true);
  ...
  if (quantity > maxPossible) {
    ...
    setClientOrderMessage(`Nie można dodać: ${result.shortages[0] || `maksymalnie ${maxPossible} szt.`}`, true);
    submitButton.disabled = true;
    return;
  }

  setClientOrderMessage(`Można dodać. Dla tej konfiguracji zostało maksymalnie ${maxPossible} szt.`, false);
...
function addToCart(flavorId, nicotineType, strength, quantity) {
  ...
  const nextCart = buildNextCart(clientCart, flavor, nicotineType, strength, quantity);
...
function bindClientOrderModal() {
  ...
  if (!modal || !closeBtn || !backdrop || !form || !typeInput || !strengthInput || !quantityInput) {
    return;
  }
  ...
  form.addEventListener('submit', (event) => {
    ...
    const nicotineType = document.getElementById('clientOrderType').value;
    const strength = Number(document.getElementById('clientOrderStrength').value);
    const quantity = Number(document.getElementById('clientOrderQuantity').value);
    const remainingResources = getRemainingResources(clientCart, getData());
    const maxPossible = getFlavorRecipeCapacity(activeOrderFlavorId, nicotineType, strength, remainingResources);

    if (!Number.isInteger(quantity) || quantity <= 0) {
      setClientOrderMessage('Błąd: ilość musi być większa od zera.', true);
      return;
    }
    ...
    const added = addToCart(activeOrderFlavorId, nicotineType, strength, quantity);
...
function renderClientFlavors() {
  ...
  data.flavors.forEach((flavor) => {
    const productionState = getFlavorProductionState(flavor.id, data, clientCart);
    const stockStatus = getStockStatus(productionState.maxPossible);
    const card = document.createElement('article');
    card.className = `card client-product-card ${stockStatus.cardClassName}`.trim();
    card.innerHTML = `
...
function buildOrderShareMessage() {
  ...
  const result = getCartRequirements(clientCart, data);
  const lines = [
    'Cześć, do przygotowania mam takie zamówienie:',
    '',
  ];

  clientCart.forEach((item, index) => {
    const typeLabel = item.nicotineType === 'salt' ? 'Sól nikotynowa' : 'Nikotyna';
    lines.push(`${index + 1}. ${item.flavorName} | ${typeLabel} | ${item.strength} mg | ${item.quantity} szt.`);
  });
