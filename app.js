...
function getSaleUnitPrice(nicotineType, strength) {
  const byType = PRICE_LIST[nicotineType];
  if (!byType) {
    return 0;
  }
...
function bindSaleModal() {
  ...
  document.querySelector('[data-close-sale-modal="true"]').addEventListener('click', closeSaleModal);
}
...
function bindSaleForm() {
  ...
  form.addEventListener('submit', (event) => {
    ...
    const saleQuantity = Number(document.getElementById('saleQuantity').value);
    ...
    if (!selectedNicotine || Number(selectedNicotine.quantity) < nicotineRequired) {
    ...
    const updatedFlavors = data.flavors.map((item) => {
      ...
      return {
        ...item,
        quantity: Number((Number(item.quantity) - flavorRequired).toFixed(2)),
      };
    });
    ...
    const updatedInventory = data.inventory.map((item) => {
      if (item.id === nicotineType) {
        return {
          ...item,
          quantity: Number((Number(item.quantity) - nicotineRequired).toFixed(2)),
        };
      }
      ...
      if (item.id === 'base') {
        return {
          ...item,
          quantity: Number((Number(item.quantity) - baseRequired).toFixed(2)),
        };
      }
      ...
      if (item.id === 'bottles') {
        return {
          ...item,
          quantity: Number((Number(item.quantity) - bottlesRequired).toFixed(2)),
        };
      }
    ...
    message.textContent = 'Sprzedaż została dodana, a stany zostały zaktualizowane.';
    renderDashboard();
    populateSaleFlavorOptions();
