...
function renderFlavorInventory() {
  ...
  data.flavors.forEach((flavor) => {
    ...
    row.querySelector('[data-flavor-quantity-id]').addEventListener('click', () => {
      handleFlavorQuantityEdit(flavor.id);
    });

    row.querySelector('[data-flavor-details-id]').addEventListener('click', () => {
      handleFlavorDetailsEdit(flavor.id);
    });

    row.querySelector('[data-flavor-delete-id]').addEventListener('click', () => {
...
function handleFlavorQuantityEdit(flavorId) {
  ...
  if (!isValidFlavorQuantity(newQuantity)) {
    showMagazynPopup('Błąd: ilość smaku musi być wielokrotnością 10.', true);
    return;
  }
...
function bindNewFlavorForm() {
  ...
  form.addEventListener('submit', (event) => {
    ...
    const descriptionInput = document.getElementById('newFlavorDescription');
    ...
    const unit = unitInput.value.trim() || 'ml';
    ...
    const data = getData();
    const nextId = data.flavors.reduce((maxId, flavor) => Math.max(maxId, Number(flavor.id) || 0), 0) + 1;
...
function bindSaveInventory() {
  ...
  saveBtn.addEventListener('click', () => {
    ...
    const updatedInventory = data.inventory.map((item) => {
      const input = Array.from(inputs).find((el) => el.dataset.id === item.id);
